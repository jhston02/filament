...
type e = Bookshelf_created of t | Bookshelf_deleted of bookshelf_event | Bookshelf_renamed of bookshelf_event * string * string | Book_added of bookshelf_event * book_id | Book_removed of bookshelf_event * book_id
...
let rename (bookshelf: t) name =
  match bookshelf.deleted with
  | true -> Error "Bookshelf already deleted"
  | false -> Ok ([Bookshelf_renamed ({ owner_id = bookshelf.owner_id; bookshelf_id = bookshelf.id },bookshelf.name, name)])
...
let evolve bookshelf event =
  let evolve_impl bookshelf event =
    ...
    | Bookshelf_renamed (_, _, new_name) -> {bookshelf with name = new_name} 
  ...
  module Private = struct
    let create_bookshelf_created_event bookshelf_id owner_id name = Bookshelf_created { id = bookshelf_id; owner_id; books = []; name = name; deleted = false }
    ...
    let create_bookshelf_renamed_event bookshelf_id owner_id old_name name = Bookshelf_renamed ({bookshelf_id; owner_id},old_name, name)
  
    let create_bookshelf bookshelf_id owner_id name deleted = {id=bookshelf_id; owner_id; name; deleted; books=[]}
    let create_bookshelf_with_book bookshelf_id owner_id name book = {id = bookshelf_id; owner_id; name; deleted=false; books=[book]}
  end
